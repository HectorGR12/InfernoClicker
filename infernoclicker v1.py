...
import win32api, win32con, win32gui, win32process, psutil, time, threading, random, winsound, os, json, subprocess, sys, \
    asyncio, itertools, pyMeow, base64, re
...
class configListener(dict):  # Detecting changes to config
    ...
    def __setitem__(self, item, value):
        ...
        if infernoClass.config["misc"]["saveSettings"]:
            json.dump(infernoClass.config, open(f"{os.environ['LOCALAPPDATA']}\\temp\\{hwid}", "w", encoding="utf-8"),
                      indent=4)
...
class inferno():
    ...
    def discordRichPresence(self):
        ...
        states = [
            ...
            "Best MantheClicker free alternative"
        ]

        while True:
            if self.config["misc"]["discordRichPresence"]:
                discordRPC.update(state=random.choice(states), start=startTime, large_image="logo",
                                  large_text="Best MantheClicker free alternative",
                                  buttons=[{"label": "Website", "url": "https://github.com/B4mb0u/Sharp"}])
            else:
                discordRPC.clear()
    ...
    def overlay(self):
        ...
        pyMeow.overlay_init(title="InfernoClicker V1")
        ...
        while pyMeow.overlay_loop():
            ...
            # Modules
            pyMeow.draw_text(f"Left", rect[0] + self.config["overlay"]["x"] + 2,
                             rect[1] + self.config["overlay"]["y"] + 2, 15, pyMeow.get_color("orange"))
            pyMeow.draw_text(f"Right", rect[0] + self.config["overlay"]["x"] + 2,
                             rect[1] + self.config["overlay"]["y"] + 16.9, 15, pyMeow.get_color("orange"))

            # Modules Status
            pyMeow.draw_text(f"[Enabled]" if self.config["left"]["enabled"] else "[Disabled]",
                             rect[0] + self.config["overlay"]["x"] + 40, rect[1] + self.config["overlay"]["y"] + 2, 15,
                             pyMeow.get_color("green") if self.config["left"]["enabled"] else pyMeow.get_color("red"))
            pyMeow.draw_text(f"[Enabled]" if self.config["right"]["enabled"] else "[Disabled]",
                             rect[0] + self.config["overlay"]["x"] + 40, rect[1] + self.config["overlay"]["y"] + 16.9,
                             15,
                             pyMeow.get_color("green") if self.config["right"]["enabled"] else pyMeow.get_color("red"))
    ...
    def leftClick(self, focused):
        if focused != None:
            ...
            if self.config["left"]["blockHit"] or (
                    self.config["left"]["blockHit"] and self.config["right"]["enabled"] and self.config["right"][
                "LMBLock"] and not win32api.GetAsyncKeyState(0x02) < 0):
...
if __name__ == "__main__":
    try:
        ...
        (suppost_sid, error) = subprocess.Popen("wmic useraccount where name='%username%' get sid",
        ...
        infernoClass = inferno(hwid)
        dpg.create_context()


        def toggleLeftClicker(id: int, value: bool):
            infernoClass.config["left"]["enabled"] = value
        ...
        def setBindLeftClicker(id: int, value: str):
            ...
            if waitingForKeyLeft:
                infernoClass.config["left"]["bind"] = value
        ...
        def setLeftMode(id: int, value: str):
            infernoClass.config["left"]["mode"] = value


        def setLeftAverageCPS(id: int, value: int):
            infernoClass.config["left"]["averageCPS"] = value


        def toggleLeftOnlyWhenFocused(id: int, value: bool):
            infernoClass.config["left"]["onlyWhenFocused"] = value


        def toggleLeftBreakBlocks(id: int, value: bool):
            infernoClass.config["left"]["breakBlocks"] = value


        def toggleLeftRMBLock(id: int, value: bool):
            infernoClass.config["left"]["RMBLock"] = value


        def toggleLeftBlockHit(id: int, value: bool):
            infernoClass.config["left"]["blockHit"] = value


        def setLeftBlockHitChance(id: int, value: int):
            infernoClass.config["left"]["blockHitChance"] = value


        def toggleLeftShakeEffect(id: int, value: bool):
            infernoClass.config["left"]["shakeEffect"] = value


        def setLeftShakeEffectForce(id: int, value: int):
            infernoClass.config["left"]["shakeEffectForce"] = value


        def setLeftClickSoundPath(id: int, value: str):
            infernoClass.config["left"]["soundPath"] = value


        def toggleLeftWorkInMenus(id: int, value: bool):
            infernoClass.config["left"]["workInMenus"] = value


        def toggleLeftBlatantMode(id: int, value: bool):
            inferno.config["left"]["blatant"] = value


        def toggleRightClicker(id: int, value: bool):
            infernoClass.config["right"]["enabled"] = value
        ...
        def setBindRightClicker(id: int, value: str):
            ...
            if waitingForKeyRight:
                infernoClass.config["right"]["bind"] = value
        ...
        def setRightMode(id: int, value: str):
            infernoClass.config["right"]["mode"] = value


        def setRightAverageCPS(id: int, value: int):
            infernoClass.config["right"]["averageCPS"] = value


        def toggleRightOnlyWhenFocused(id: int, value: int):
            infernoClass.config["right"]["onlyWhenFocused"] = True


        def toggleRightLMBLock(id: int, value: bool):
            infernoClass.config["right"]["LMBLock"] = value


        def toggleRightShakeEffect(id: int, value: bool):
            infernoClass.config["right"]["shakeEffect"] = value


        def setRightShakeEffectForce(id: int, value: int):
            infernoClass.config["right"]["shakeEffectForce"] = value


        def setRightClickSoundPath(id: int, value: str):
            infernoClass.config["right"]["soundPath"] = value


        def toggleRightWorkInMenus(id: int, value: bool):
            infernoClass.config["right"]["workInMenus"] = value


        def toggleRightBlatantMode(id: int, value: bool):
            infernoClass.config["right"]["blatant"] = value


        def toggleRecorder(id: int, value: bool):
            infernoClass.config["recorder"]["enabled"] = value
            ...
            while True:
                if not recording:
                    if len(recorded) < 2:  # Avoid saving a record with 0 click
                        ...
                        del recorded[
                            -1]  # Deleting last record time because that's when you click on stop button and it can take some time

                    infernoClass.config["recorder"]["record"] = recorded

                    infernoClass.record = itertools.cycle(recorded)

                    totalTime = 0
                    for clickTime in recorded:
                        totalTime += float(clickTime)

                    dpg.set_value(averageRecordCPSText,
                                  f"Average CPS of previous Record: {round(len(recorded) / totalTime, 2)}")
        ...
        def setBindHideGUI(id: int, value: str):
            ...
            if waitingForKeyHideGUI:
                infernoClass.config["misc"]["bindHideGUI"] = value
        ...
        def toggleOverlay(id: int, value: bool):
            infernoClass.config["overlay"]["enabled"] = value

            if value:
                threading.Thread(target=infernoClass.overlay, daemon=True).start()


        def toggleOverlayOnlyWhenFocused(id: int, value: bool):
            infernoClass.config["overlay"]["onlyWhenFocused"] = value


        def setOverlayX(id: int, value: int):
            infernoClass.config["overlay"]["x"] = value


        def setOverlayY(id: int, value: int):
            infernoClass.config["overlay"]["y"] = value


        def toggleSaveSettings(id: int, value: bool):
            infernoClass.config["misc"]["saveSettings"] = value


        def toggleAlwaysOnTop(id: int, value: bool):
            if value:
                win32gui.SetWindowPos(guiWindows, win32con.HWND_TOPMOST, 0, 0, 0, 0,
                                      win32con.SWP_NOMOVE | win32con.SWP_NOSIZE)
            else:
                win32gui.SetWindowPos(guiWindows, win32con.HWND_NOTOPMOST, 0, 0, 0, 0,
                                      win32con.SWP_NOMOVE | win32con.SWP_NOSIZE)


        def toggleDiscordRPC(id: int, value: bool):
            infernoClass.config["misc"]["discordRichPresence"] = value


        dpg.create_viewport(title=f"[v{version}] Inferno  Clicker V1", width=810, height=435)
            ...
            with dpg.tab_bar():
                with dpg.tab(label="Left Clicker"):
                    ...
                    with dpg.group(horizontal=True):
                        checkboxToggleLeftClicker = dpg.add_checkbox(label="Toggle",
                                                                     default_value=infernoClass.config["left"]["enabled"],
                                                                     callback=toggleLeftClicker)
                        buttonBindLeftClicker = dpg.add_button(label="Click to Bind", callback=statusBindLeftClicker)
                        dropdownLeftMode = dpg.add_combo(label="Mode", items=["Hold", "Always"],
                                                         default_value=infernoClass.config["left"]["mode"],
                                                         callback=setLeftMode)

                        bind = infernoClass.config["left"]["bind"]
                        if bind != 0:
                            dpg.set_item_label(buttonBindLeftClicker, f"Bind: {chr(bind)}")

                    dpg.add_spacer(width=75)

                    sliderLeftAverageCPS = dpg.add_slider_int(label="Average CPS",
                                                              default_value=infernoClass.config["left"]["averageCPS"],
                                                              min_value=1, callback=setLeftAverageCPS)

                    dpg.add_spacer(width=75)
                    dpg.add_separator()
                    dpg.add_spacer(width=75)

                    checkboxLeftOnlyWhenFocused = dpg.add_checkbox(label="Only In Game",
                                                                   default_value=infernoClass.config["left"][
                                                                       "onlyWhenFocused"],
                                                                   callback=toggleLeftOnlyWhenFocused)

                    dpg.add_spacer(width=75)

                    checkBoxLeftBreakBlocks = dpg.add_checkbox(label="Break Blocks",
                                                               default_value=infernoClass.config["left"]["breakBlocks"],
                                                               callback=toggleLeftBreakBlocks)

                    dpg.add_spacer(width=75)

                    checkboxLeftRMBLock = dpg.add_checkbox(label="RMB-Lock",
                                                           default_value=infernoClass.config["left"]["RMBLock"],
                                                           callback=toggleLeftRMBLock)

                    dpg.add_spacer(width=125)

                    checkboxLeftBlockHit = dpg.add_checkbox(label="BlockHit",
                                                            default_value=infernoClass.config["left"]["blockHit"],
                                                            callback=toggleLeftBlockHit)
                    sliderLeftBlockHitChance = dpg.add_slider_int(label="BlockHit Chance",
                                                                  default_value=infernoClass.config["left"][
                                                                      "blockHitChance"], min_value=1, max_value=100,
                                                                  callback=setLeftBlockHitChance)

                    dpg.add_spacer(width=125)

                    checkboxLeftShakeEffect = dpg.add_checkbox(label="Shake Effect",
                                                               default_value=infernoClass.config["left"]["shakeEffect"],
                                                               callback=toggleLeftShakeEffect)
                    sliderLeftShakeEffectForce = dpg.add_slider_int(label="Shake Effect Force",
                                                                    default_value=infernoClass.config["left"][
                                                                        "shakeEffectForce"], min_value=1, max_value=20,
                                                                    callback=setLeftShakeEffectForce)

                    dpg.add_spacer(width=75)
                    dpg.add_separator()
                    dpg.add_spacer(width=75)

                    inputLeftClickSoundPath = dpg.add_input_text(label="Click Sound Path (empty for no sound)",
                                                                 default_value=infernoClass.config["left"]["soundPath"],
                                                                 hint="Exemple: mysounds/G505.wav",
                                                                 callback=setLeftClickSoundPath)

                    dpg.add_spacer(width=75)
                    dpg.add_separator()
                    dpg.add_spacer(width=75)

                    checkboxLeftWorkInMenus = dpg.add_checkbox(label="Work in Menus",
                                                               default_value=infernoClass.config["left"]["workInMenus"],
                                                               callback=toggleLeftWorkInMenus)
                    checkboxLeftBlatantMode = dpg.add_checkbox(label="Blatant Mode",
                                                               default_value=infernoClass.config["left"]["blatant"],
                                                               callback=toggleLeftBlatantMode)

                with dpg.tab(label="Right Clicker"):
                    dpg.add_spacer(width=75)

                    with dpg.group(horizontal=True):
                        checkboxToggleRightClicker = dpg.add_checkbox(label="Toggle",
                                                                      default_value=infernoClass.config["right"][
                                                                          "enabled"], callback=toggleRightClicker)
                        buttonBindRightClicker = dpg.add_button(label="Click to Bind", callback=statusBindRightClicker)
                        dropdownRightMode = dpg.add_combo(label="Mode", items=["Hold", "Always"],
                                                          default_value=infernoClass.config["right"]["mode"],
                                                          callback=setRightMode)

                        bind = infernoClass.config["right"]["bind"]
                        if bind != 0:
                            dpg.set_item_label(buttonBindRightClicker, f"Bind: {chr(bind)}")

                    dpg.add_spacer(width=75)

                    sliderRightAverageCPS = dpg.add_slider_int(label="Average CPS",
                                                               default_value=infernoClass.config["right"]["averageCPS"],
                                                               min_value=1, callback=setRightAverageCPS)

                    dpg.add_spacer(width=75)
                    dpg.add_separator()
                    dpg.add_spacer(width=75)

                    checkboxRightOnlyWhenFocused = dpg.add_checkbox(label="Only In Game",
                                                                    default_value=infernoClass.config["right"][
                                                                        "onlyWhenFocused"],
                                                                    callback=toggleRightOnlyWhenFocused)

                    dpg.add_spacer(width=75)

                    checkboxRightLMBLock = dpg.add_checkbox(label="LMB-Lock",
                                                            default_value=infernoClass.config["right"]["LMBLock"],
                                                            callback=toggleRightLMBLock)

                    dpg.add_spacer(width=75)

                    checkboxRightShakeEffect = dpg.add_checkbox(label="Shake Effect",
                                                                default_value=infernoClass.config["right"]["shakeEffect"],
                                                                callback=toggleRightShakeEffect)
                    sliderRightShakeEffectForce = dpg.add_slider_int(label="Shake Effect Force",
                                                                     default_value=infernoClass.config["right"][
                                                                         "shakeEffectForce"], min_value=1, max_value=20,
                                                                     callback=setRightShakeEffectForce)

                    dpg.add_spacer(width=75)
                    dpg.add_separator()
                    dpg.add_spacer(width=75)

                    inputRightClickSoundPath = dpg.add_input_text(label="Click Sound Path (empty for no sound)",
                                                                  default_value=infernoClass.config["right"]["soundPath"],
                                                                  hint="Exemple: mysounds/G505.wav",
                                                                  callback=setRightClickSoundPath)

                    dpg.add_spacer(width=75)
                    dpg.add_separator()
                    dpg.add_spacer(width=75)

                    checkboxRightWorkInMenus = dpg.add_checkbox(label="Work in Menus",
                                                                default_value=infernoClass.config["right"]["workInMenus"],
                                                                callback=toggleRightWorkInMenus)
                    checkboxRightBlatantMode = dpg.add_checkbox(label="Blatant Mode",
                                                                default_value=infernoClass.config["right"]["blatant"],
                                                                callback=toggleRightBlatantMode)
                with dpg.tab(label="Recorder"):
                    dpg.add_spacer(width=75)

                    recorderInfoText = dpg.add_text(
                        default_value="Records your legit way of clicking in order to produce clicks even less detectable by AntiCheat.\nAfter pressing the \"Start\" button, click as if you were in PvP for a few seconds. Then press the \"Stop\" button.\nOnly works for the left click.")

                    dpg.add_spacer(width=75)
                    dpg.add_separator()
                    dpg.add_spacer(width=75)

                    checkboxRecorderEnabled = dpg.add_checkbox(label="Enabled",
                                                               default_value=infernoClass.config["recorder"]["enabled"],
                    ...
                    totalTime = 0
                    for clickTime in infernoClass.config["recorder"]["record"]:
                        totalTime += float(clickTime)

                    dpg.set_value(averageRecordCPSText,
                                  f"Average CPS of previous Record: {round(len(infernoClass.config['recorder']['record']) / totalTime, 2)}")
                ...
                with dpg.tab(label="Misc"):
                    ...
                    with dpg.group(horizontal=True):
                        ...
                        bind = infernoClass.config["misc"]["bindHideGUI"]
                        if bind != 0:
                    ...
                    overlayInfoText = dpg.add_text(
                        default_value="Displays an in-game GUI with Sharp's modules status info. Useful but does not work in fullscreen mode (F11)!")

                    with dpg.group(horizontal=True):
                        overlayToggle = dpg.add_checkbox(label="Overlay",
                                                         default_value=infernoClass.config["overlay"]["enabled"],
                                                         callback=toggleOverlay)

                        if infernoClass.config["overlay"]["enabled"]:
                            toggleOverlay(1, True)

                        overlayOnlyWhenFocusedToggle = dpg.add_checkbox(
                            label="Only when game focused (easier to change overlay position when disabled)",
                            default_value=infernoClass.config["overlay"]["onlyWhenFocused"],
                            callback=toggleOverlayOnlyWhenFocused)

                    overlayXSlider = dpg.add_slider_int(label="X Position",
                                                        default_value=infernoClass.config["overlay"]["x"], min_value=0,
                                                        max_value=1920, callback=setOverlayX)
                    overlayYSlider = dpg.add_slider_int(label="Y Position",
                                                        default_value=infernoClass.config["overlay"]["y"], min_value=0,
                                                        max_value=1080, callback=setOverlayY)

                    dpg.add_spacer(width=75)
                    dpg.add_separator()
                    dpg.add_spacer(width=75)

                    saveSettings = dpg.add_checkbox(label="Save Settings",
                                                    default_value=infernoClass.config["misc"]["saveSettings"],
                    ...
                    checkboxAlwaysOnTop = dpg.add_checkbox(label="Discord Rich Presence",
                                                           default_value=infernoClass.config["misc"][
                                                               "discordRichPresence"], callback=toggleDiscordRPC)

                    dpg.add_spacer(width=75)
                    dpg.add_separator()
                    dpg.add_spacer(width=75)

                    creditsText = dpg.add_text(default_value="Credits: Hector.GR12 (Developer)")
                    githubText = dpg.add_text(default_value="https://github.com/HectorGR12")
        ...
        with dpg.theme() as global_theme:
            with dpg.theme_component(dpg.mvAll):
                ...
                dpg.add_theme_color(dpg.mvThemeCol_TabActive, (55, 168, 0))
                dpg.add_theme_color(dpg.mvThemeCol_TabHovered, (55, 168, 0))
                dpg.add_theme_color(dpg.mvThemeCol_ButtonHovered, (55, 168, 0))
                dpg.add_theme_color(dpg.mvThemeCol_CheckMark, (55, 168, 0))
                dpg.add_theme_color(dpg.mvThemeCol_ScrollbarGrabHovered, (55, 168, 0))
                dpg.add_theme_color(dpg.mvThemeCol_ScrollbarGrabActive, (55, 168, 0))
                dpg.add_theme_color(dpg.mvThemeCol_SliderGrab, (55, 168, 0))
                dpg.add_theme_color(dpg.mvThemeCol_SliderGrabActive, (55, 168, 0))
